...
CREATE TABLE `payment_methods` (
  `payment_method_id` tinyint(4) PRIMARY KEY NOT NULL,
...
CREATE TABLE `payments` (
  ...
  FOREIGN KEY (`payment_method`) REFERENCES `payment_methods` (`payment_method_id`) ON UPDATE CASCADE
);
...
CREATE TABLE `shippers` (
  `shipper_id` smallint(6) PRIMARY KEY NOT NULL,
  `name` varchar(50) NOT NULL
);
...
CREATE TABLE `order_statuses` (
  `order_status_id` tinyint(4) PRIMARY KEY NOT NULL,
  `name` varchar(50) NOT NULL
);
...
CREATE TABLE `offices` (
  `office_id` int(11) PRIMARY KEY NOT NULL,
  `address` varchar(50) NOT NULL,
  `city` varchar(50) NOT NULL,
...
CREATE TABLE `employees` (
  ...
  FOREIGN KEY (`reports_to`) REFERENCES `employees` (`employee_id`) ON UPDATE CASCADE,
